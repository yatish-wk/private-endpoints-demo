...
function link_private_zone_to_vnet() {
    ...
    if az network private-dns link vnet show \
        ...
        az network private-dns link vnet create --name $linkName --resource-group $resourceGroup --zone-name $privateDnsZoneName --virtual-network $vnetName --registration-enabled false 
